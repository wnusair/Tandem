...
/// Decrypt a WASM blob using AES-256-GCM, given the raw 32-byte DEK, the
/// base64-encoded 12-byte IV, and `ciphertext || auth-tag`.
pub fn decrypt_blob(
    dek: &[u8],
    iv_b64: &str,
    ciphertext: &[u8],
...
/// SHA-256 hash of `data` as raw bytes, for seeding rather than printing.
